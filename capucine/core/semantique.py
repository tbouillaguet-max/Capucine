"""Ce que Capucine sait de vos documents et de vos conversations.
...
* **Aucune dépendance nouvelle.** Les vecteurs vivent en BLOB dans le SQLite
  ...
  dizaines de milliers de fragments, il faudra autre chose — et Capucine le
...
from .errors import CapucineError
...
class IndexPlein(CapucineError):
    """L'index a atteint la taille au-delà de laquelle le balayage traîne.

    Une erreur explicite plutôt qu'un ralentissement progressif : Capucine
...
class Connaissances:
    ...
    def empreinte(chemin: str | Path) -> str:
        ...
        Exposée ici pour qu'un plugin n'ait à importer que ``capucine.plugin``.
        """
    ...
    def indexer_le_tour(self, reference: str, question: str, reponse: str) -> None:
        ...
        vaut mieux qu'une Capucine qui attend.
        """
    ...
    def _demarrer_le_fil(self) -> None:
        ...
        self._fil = threading.Thread(
            target=self._boucle_de_fond, name="capucine-index", daemon=True
        )
        self._fil.start()
