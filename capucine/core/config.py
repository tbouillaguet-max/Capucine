...
3. les variables d'environnement ``CAPUCINE_<SECTION>__<CLE>`` ;
...
def _env_overlay(environ: Mapping[str, str]) -> dict[str, Any]:
    """``CAPUCINE_LLM__MODEL=qwen`` -> ``{"llm": {"model": "qwen"}}``.

    Le double tiret bas sépare les niveaux, un simple tiret bas reste dans le
    nom de la clé (``CAPUCINE_AUDIO__SAMPLE_RATE`` -> ``audio.sample_rate``).
    """
