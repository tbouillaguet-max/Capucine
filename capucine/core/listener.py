...
fermé, même pendant que Capucine parle. Selon le mode courant, chaque trame
...
class ListenMode(StrEnum):
    ...
    WAKE = "eveil"              # on attend « Capucine »
    UTTERANCE = "enonce"        # on capture ce qui est dit
    MONITOR = "surveillance"    # Capucine parle : on guette une interruption


class BargeInMode(StrEnum):
    OFF = "off"                 # on ne coupe jamais la parole de Capucine
    VOICE = "voix"              # toute parole soutenue l'interrompt
    WAKE = "eveil"              # seul « Capucine » l'interrompt (robuste à l'écho)
