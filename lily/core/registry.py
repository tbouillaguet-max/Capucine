...
* **Isolation des pannes.** Un plugin qui lève à l'import est écarté avec un
  ...
  est neutralisé, et Lily répond qu'elle n'a pas pu exécuter la commande.
...
boucle infinie sera *abandonné* — Lily répond et le met en quarantaine —
...
def _module_name_for(path: Path) -> str:
    ...
    module est déposé dans un espace de noms virtuel ``lily.plugins``.
    """
...
def _ensure_namespace() -> None:
    """Crée l'espace de noms virtuel ``lily.plugins``.
...
def run_with_timeout(
    ...
    Le thread est démon pour qu'un plugin bloqué n'empêche jamais Lily de
...
class PluginRegistry:
    """Le registre : ce que Lily sait faire, à un instant donné."""
    ...
    def __init__(
        ...
        *,
        ...
        self.data_root = Path(data_root) if data_root else Path.home() / ".lily" / "data"
    ...
    def discover_files(self) -> list[Path]:
        ...
        antérieur : ``~/.lily/plugins`` peut ainsi surcharger un plugin
    ...
    def _import_module(self, path: Path, module_name: str) -> types.ModuleType:
        ...
        except BaseException as exc:  # noqa: BLE001 - un plugin ne fait jamais tomber Lily
