"""L'atelier : la seule porte par laquelle Lily touche à vos fichiers.
...
class AtelierError(SkillRefused, PermissionError):
    ...
    Lily doit le prononcer mot pour mot.
    """


@dataclass
class Atelier:
    """Les dossiers que Lily a le droit de lire et d'écrire."""
    ...
    def jeter(self, chemin: str | Path) -> Path:
        ...
        cible = self.resoudre(chemin, doit_exister=True)
        self.verifier_ecriture(cible)
        corbeille = (self.corbeille or Path.home() / ".lily" / "corbeille")
