"""La mémoire de Lily : ce qu'elle garde entre deux démarrages.
...
Tout est local, dans ``~/.lily/memoire.sqlite``. Rien ne sort de la
...
def depuis_config(config) -> Memoire | None:
    ...
    if not bool(section.get("active", True)):
        ...
        return None
    chemin = config.resolve_path("memoire.fichier") or Path.home() / ".lily" / "memoire.sqlite"
