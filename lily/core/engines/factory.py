...
LLM_ENGINES: dict[str, tuple[str, str]] = {
    "mock": ("lily.core.engines.llm.mock", "MockLLM"),
    "ollama": ("lily.core.engines.llm.ollama", "OllamaLLM"),
    "llamacpp": ("lily.core.engines.llm.llamacpp", "LlamaCppLLM"),
}

STT_ENGINES: dict[str, tuple[str, str]] = {
    "faster-whisper": ("lily.core.engines.stt.fasterwhisper", "FasterWhisperSTT"),
    "whisper": ("lily.core.engines.stt.fasterwhisper", "FasterWhisperSTT"),
    "vosk": ("lily.core.engines.stt.vosk", "VoskSTT"),
    "scripted": ("lily.core.engines.stt.scripted", "ScriptedSTT"),
}

TTS_ENGINES: dict[str, tuple[str, str]] = {
    "piper": ("lily.core.engines.tts.piper", "PiperTTS"),
    "silent": ("lily.core.engines.tts.silent", "SilentTTS"),
}

VAD_ENGINES: dict[str, tuple[str, str]] = {
    "silero": ("lily.core.engines.vad.silero", "SileroVAD"),
    "energie": ("lily.core.engines.vad.energy", "EnergyVAD"),
    "energy": ("lily.core.engines.vad.energy", "EnergyVAD"),
    "scripted": ("lily.core.engines.vad.scripted", "ScriptedVAD"),
}
...
WAKE_ENGINES: dict[str, tuple[str, str]] = {
    "openwakeword": ("lily.core.engines.wake.openwakeword", "OpenWakeWordEngine"),
    "vosk": ("lily.core.engines.wake.vosk", "VoskWakeWord"),
    "scripted": ("lily.core.engines.wake.scripted", "ScriptedWakeWord"),
}
...
def build_llm(config: Any) -> LLMEngine:
    ...
    Le repli est volontaire : une Lily sans modèle doit continuer à
...
def build_stt(config: Any) -> STTEngine:
    ...
    Aucun repli silencieux, contrairement au LLM : une Lily qui n'entend
    ...
    if not engine.available():
        raise EngineUnavailable(
            ...
            "modèle (python -m lily.core.downloads tout), ou utilisez --text."
        )
...
def build_tts(config: Any) -> TTSEngine | None:
    ...
    Ici le repli est légitime : sans voix, Lily affiche sa réponse au lieu
    ...
    if not engine.available():
        logger.warning(
            "Moteur de synthèse « %s » indisponible : Lily affichera ses réponses "
            "au lieu de les dire. Téléchargez la voix avec : "
            "python -m lily.core.downloads voix",
            engine.describe(),
        )
        return None
...
def build_audio_output(
    ...
    if not sortie.available():
        logger.warning(
            "Aucun haut-parleur utilisable : Lily affichera ses réponses au lieu "
...
def build_wake(config: Any) -> WakeWordEngine | None:
    ...
    « lily » doit être entraîné, ce qui prend du temps ; tant qu'il
    n'existe pas, Vosk à grammaire restreinte prend le relais. Si aucun des
    deux n'est disponible, on rend ``None`` et Lily écoute en permanence,
    ...
    if name == "openwakeword":
        ...
        logger.warning(
            ...
            "Pour l'entraîner : python tools/entrainer_lily.py --preparer",
            section.get("word", "lily"),
        )
    ...
    logger.warning(
        "Aucun détecteur de mot d'éveil disponible : Lily écoutera en "
